...
output_filename = f'/ailab/user/huanghaifeng/work/robocasa_exps_haifeng/robocasa/datasets/v0.1/generated_data/{env_name}.json'
cnt = 0
...
for demo_id in tqdm(f['data']):
    try:
        ...
        # 生成spatial instructions
        # todo: 限定类别是唯一的；多种情况都保存一下
        obj_pos_flag = False
    ...
    #     image_data = f[f'data/{demo_id}/obs/{view_name}']
    ...
    #     # 使用 PIL 保存图片
    #     output_path = f"./output/{demo_id}_{view_name}.jpg"
    #     pil_image.save(output_path)

    # for mask_name in mask_names:
    #     mask_data = f[f'data/{demo_id}/obs/{mask_name}']
    #     first_mask = mask_data[0]
    #     print(first_mask.shape)

    #     pil_mask = Image.fromarray(first_mask)        
    #     output_path = f"./output/{demo_id}_{mask_name}.jpg"
